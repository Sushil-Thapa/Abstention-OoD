...
def train_epoch_dual_dac(epoch, loader1, loader2, model, loss2, optimizer, scheduler, args):
    ...
    for i, (inputs, targets) in enumerate(loader1):
        ...
        n_class = model.children().__next__().linear.out_features
        inputs = torch.cat((inputs, inputs2), 0)

        targets2 = (n_class - 1) * torch.ones(inputs2.shape[0], dtype=torch.int64) # Push targets as t < n_class
        ...
        # evaluate
        acc = accuracy(outputs[:N], targets[:N], args.topk) # TODOsu for DAC, Might have to evaluation for all outputs
        for j in range(len(accs)):
            accs[j].update(acc[j], inputs.size(0))
